...
class MOMBase(ClassifierMixin, BaseEstimator):
    ...
    def penalty(self, val):
        if val not in MOMBase._penalties:
            raise ValueError(
    ...
    def strategy(self, val):
        if val not in MOMBase._strategies:
            raise ValueError(
    ...
    @solver.setter
    def solver(self, val):
        if val not in MOMBase._solvers:
            raise ValueError(
    ...
    def thresholding(self, val):
        if not isinstance(val, bool):
            raise ValueError("thresholding must be True or False; got (thresholding=%r)" % val)
        else:
            self._thresholding = val
...
class BinaryClassifier(MOMBase, ClassifierMixin):
...
class MOMRegressor(MOMBase, RegressorMixin):
    ...
    def predict(self, X):
        ...
        # For now, no sparse arrays
        # X = check_array(X, accept_sparse="csr")
        X = check_array(X, accept_sparse=False, estimator="MOMRegressor")
